...
@torch.no_grad()
def validate_combined(model, iters=32):
    ...
    for fps in [1, 4, 8, 16, 24, 30, 60]:
        ...
        for val_id in range(len(val_dataset)):
            ...
            _, flow_pre = model(image1, image2, iters=iters, test_mode=True)

            flow_pred = padder.unpad(flow_pre[0]).cpu()
            ...
            flow_pred_viz = np.pad(flow_viz.flow_to_image(flow_pred), pad_width=npad, mode='constant', constant_values=0)
            img = np.concatenate([image1, image2], axis=1)
            flo = np.concatenate([flow_gt_viz, flow_pred_viz], axis=1)
        ...
        print("Validation SimulatedCombined FPS:(%s) EPE: %f, 1px: %f, 3px: %f, 5px: %f" % (fps, epe, px1, px3, px5))
