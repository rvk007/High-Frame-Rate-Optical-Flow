...
class FlowAugmentor:
    ...
    def color_transform(self, imgs):
        ...
        # symmetric
            ...
            image_stack = np.concatenate([img for img in imgs], axis=0)
    ...
    def __call__(self, imgs, flow):
        imgs = self.color_transform(imgs)
        imgs[-2], imgs[-1] = self.eraser_transform(imgs[-2], imgs[-1])
        if self.pwc_aug:
            # FIXME: update spatial_augmentor for multiple images
            ...
            imgs, flow = self.spatial_transform(imgs, flow)
