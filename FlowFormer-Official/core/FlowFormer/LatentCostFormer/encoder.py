...
class MemoryEncoder(nn.Module):
    def __init__(self, cfg):
        ...
        if cfg.fnet == 'twins':
            ...
            self.feat_encoder = BasicEncoder(input_dim=12, output_dim=256, norm_fn='instance')
        else:
            exit()
    ...
    # def forward(self, imgs, data, context=None):
    #     # The original implementation
    #     # feat_s = self.feat_encoder(img1)
    #     # feat_t = self.feat_encoder(img2)
    #     # feat_s = self.channel_convertor(feat_s)
    ...
    #     # imgs = torch.cat([img1, img2], dim=0)
    #     print('In MemoryEncoder')
    #     batch_size = imgs[0].shape[0]
    #     print('batch_size',batch_size)
    #     imgs = imgs[:2]
    #     imgs = torch.cat(imgs, dim=0)
    #     print('After cat imgs',imgs.shape)
    #     feats = self.feat_encoder(imgs)
    #     print('Before feats',feats.shape)
    #     feats = self.channel_convertor(feats)
    #     print('After channel_convertor feats',feats.shape)
    #     B = feats.shape[0] // 2
    ...
    #     cost_volume = self.corr(feat_s, feat_t)
    #     print('Before cost_volume', cost_volume.shape)

    #     # FIXME: Find a better a day to do this
    #     max_values = []
    #     for i in range(batch_size):
    #         max_values.append(torch.max(cost_volume, 0, keepdim=True)[i])
    #     cost_volume = torch.cat(max_values, dim=0)
    #     print('After cost_volume', cost_volume.shape)
    #     x = self.cost_perceiver_encoder(cost_volume, data, context)
    #     print('After cost_perceiver_encoder', x.shape)
