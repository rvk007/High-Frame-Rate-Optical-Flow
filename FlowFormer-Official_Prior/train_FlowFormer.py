...
def train(cfg):
    args.name += '_weighting_' if args.weighting else '_no_weighting_'
    args.name += 'svt' if args.use_prior else 'basicencoder'
    args.name = args.name + '_2_gpus'
    cfg.latentcostformer.use_prior = 'svt' if args.use_prior else 'basicdecoder'
    run = wandb.init(
    ...
    while should_keep_training:
        for i_batch, data_blob in enumerate(train_loader):
            ...
            if total_steps % cfg.val_freq == cfg.val_freq - 1:
                # print('In validation total_steps', total_steps)
