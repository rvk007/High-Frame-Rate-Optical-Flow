...
class twins_svt_large(nn.Module):
    def __init__(self, pretrained=True, use_prior='svt', cnet=False):
        super().__init__()
        ...
        del self.svt.pos_block[2]
        # if use_prior=='svt' and cnet:
        #     self.svt.patch_embeds[0].proj = nn.Conv2d(12, 128, kernel_size=(4, 4), stride=(4, 4)).to('cuda')

    def forward(self, x, data=None, layer=2, use_prior='svt'):
        # print('In twins_svt_large forward', use_prior)
        use_prior = 'basicencoder'
        B, C, _, _ = x.shape
        if C > 3:
            # if use_prior=='svt':
            #     self.svt.patch_embeds[0].proj = nn.Conv2d(12, 128, kernel_size=(4, 4), stride=(4, 4)).to('cuda')
            if use_prior =='basicencoder':
